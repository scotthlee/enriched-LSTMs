...
from tools.generic import boot_sample, threshold
...
def brier_score_multi(true, pred):
    errors = np.array([np.sum((true[i] - pred[i])**2 
                              for i in range(true.shape[0]))])
    return np.float(np.sum(errors) / true.shape[0])
